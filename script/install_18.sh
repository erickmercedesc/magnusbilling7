...
apt-get -y install autoconf automake devscripts gawk ntpdate ntp g++ git-core curl sudo xmlstarlet  apache2 libjansson-dev git  odbcinst1debian2 libodbc1 odbcinst unixodbc unixodbc-dev 
apt-get -y install php-fpm php  php-dev php-common php-cli php-gd php-pear php-cli php-sqlite3 php-curl php-mbstring unzip libapache2-mod-php uuid-dev libxml2 libxml2-dev openssl libcurl4-openssl-dev gettext gcc g++ libncurses5-dev sqlite3 libsqlite3-dev subversion mpg123
apt-get -y install mariadb-server php-mysql
apt-get -y install unzip git libcurl4-openssl-dev htop sngrep
...
[mbilling-connector-massive-call]
Description = MySQL Asterisk
Driver = MariaDB Unicode
Database = mbilling
Server = localhost
...
[magnusbilling]
enabled = yes
dsn = mbilling-connector
username = root
password = $password
pre-connect = yes

[magnusbillingmassiveCall]
enabled = yes
dsn = mbilling-connector-massive-call
username = root
password = $password
pre-connect = yes
...
{       
    ...
    if [ "$4" == "Intel(R)" &&  "$5" == "Pentium(R)" && "$6"== "III" ];then
        ...
        mv /usr/src/codec_g723-ast18-gcc4-glibc-pentium.so /usr/lib/asterisk/modules/codec_g723.so
        mv /usr/src/codec_g729-ast18-gcc4-glibc-pentium.so /usr/lib/asterisk/modules/codec_g729.so
        return 0;
    fi
